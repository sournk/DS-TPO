...
- [x] 8. Касание зоны NPOC активирует режим ожидания слома структуры в обратную сторону (на отскок) на младшем TF:
    ...
    - [x] 8.6. Бот использует стандартный индикатор ZigZag или кастомизированный PADD-Fractal-Ind для определения вершин структуры. Поэтому в настройках можно подобрать параметры для более грубого или точного определения экстремумов.
- [x] 9. Бот входит в сделку после подтверждения слома структуры - пробития актуального уровня BOS от него.
- [x] 10. Бот устанавливает SL и TP ордера или позиции на фиксированных дистанциях от BOS.
...
- [x] ==`EntryMode` *(0.1.7)*: Режим входа в сделку:== 
    - ==`Агрессивный`: бот входит в сделку по рынку при пробитии BOS, обнаруженном на тике.==
    - ==`Консервативный` бот входит в сделку после закрытия свечи выше/ниже BOS отложенным ордером LIMIT или STOP.==
...
- [x] ==`OrderExpirationAfterSLLevel`: Отменять ордера ниже SL==
- [x] ==`OrderExpirationAfterTPLevel`: Отменять ордера выше TP==
...
- [x] ==`SPFilterForNPOCModeEnable` *(0.1.7)*: Фильтр по количеству SP для NPOC режима==
...
- [x] 4.19. Отменять отложенные ордера, если цена выше/ниже TP.

## Замечания 28.07.24
Поменять индикатор на https://www.mql5.com/en/market/product/77996. Мы погоняли его демо-версию, он показывает практически все идентичное, за исключением SP. Наш текущий индикатор за счет steps неправильно считает single prints поскольку волатильность каждый день разная, а дробление в любом случае на 30 блоков происходит.

Сделать работу с двумя индикаторами на выбор будет сложно, потому что они рисуют профили очень по-разному. Не хочу на это тратить время, если текущий не устраивает. Поэтому предлагаю заменить в боте индикатор полностью. Для парсинг профилей на графиках я переделаю фактически заново. А также заново буду искать POC, потому что в этом индикаторе нет как таковой линии POC. Есть только "квадратики" другого цвета, которые обозначают POC. Это не проблема, но придется еще POC эти по-новому искать.

Еще заметил следующее:
1. Новый индикатор сильно медленнее текущего. С виду раз в 10. Это видимо потому, что он рисует каждый "квадратик" отдельно, а каждый профиль состоит и сотен таких графических объектов. В тестере бот с ним будет работать очень медленно. На реальную торговлю это повлиять не должно. Но могут быть проблемы, если бот с этим индикатором будет торговать в терминале с большим количеством открытых графиков и других ботов. Терминал может закрываться из-за недостатка памяти или других ресурсов, когда на графике будет выведено много профилей за много дней.
2. В индикаторе 3 режима графики. Бот сможет работать только с Simple. Остальные режимы рисуют графику без реальной привязке к свечам. Распознать ее на чартах не получится. Они еще более тормозные, и как их реально использовать я не представляю. Еще настройки цветов и линий разных частей профиля в боте не будет. Я их преднастрою мной так, чтобы бот смог их "распознать" на чартах. На график с ботом в реальной торговле нельзя будет добавлять этот индикатор отдельно. Потому что бот тогда не поймет где его объекты, а где просто от добавленного индикатора.
3. В индикаторе также нет выбора TF. Он всегда работает в сессии, дне, неделе или месяце. При этом не важно какой TF на графике. Когда мы на созвоне обсуждали SP для входа, вы говорили, что смотрите индикатор на TF BOS. Но этот индикатор ничем не будет отличаться на TF POC от TF BOS или любого другого.